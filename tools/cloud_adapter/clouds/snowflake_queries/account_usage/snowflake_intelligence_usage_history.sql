...
SELECT
    ...
    u.request_id,
    u.parent_request_id,
    u.snowflake_intelligence_id,
    u.snowflake_intelligence_name,
    u.agent_database_id,
...
FROM SNOWFLAKE.ACCOUNT_USAGE.SNOWFLAKE_INTELLIGENCE_USAGE_HISTORY u
LEFT JOIN rates r
  ON r.date = DATE(u.start_time)
 AND r.account_locator = CURRENT_ACCOUNT()
 AND r.service_type = 'AI_SERVICES'
