...
export const getTaggingConditions = (formData) => {
...
export const getDefinition = (type, formData) =>
  ({
    [RESOURCE_COUNT_ANOMALY]: {
...
export const getFiltersFromFormData = (formData) => {
  const formFilters = Object.fromEntries(
    Object.entries(formData[CREATE_ORGANIZATION_CONSTRAINT_FORM_FIELD_NAMES.FILTERS]).flatMap(([key, value]) => {
      ...
      return Object.entries(apiValues).filter(([, filterValue]) => {
        // API throws an error if we pass empty array so we need to filter them out
        if (Array.isArray(filterValue)) {
          return !isEmptyArray(filterValue);
        }
...
export const buildConstraintParamsFromFormData = (formData, { includeType = true } = {}) => {
  ...
  const params = {
    ...
    filters: getFiltersFromFormData(formData),
  };
