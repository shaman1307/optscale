...
from auth.auth_server.controllers.signin import OidcOauth2Provider
...
class TestSignIn(TestAuthBase):
    ...
    def test_signin_oidc_no_client_id(self):
        code, _ = self.client.post(
            self.client.signin_url(),
            {'provider': 'oidc', 'token': 'code',
             'redirect_uri': 'https://optscale.example/login'})
        self.assertEqual(code, 403)

    @patch.dict(os.environ, {
        'OIDC_ISSUER': 'https://example.onelogin.com/oidc/2',
        'OIDC_CLIENT_ID': 'cid',
        'OIDC_CLIENT_SECRET': 'secret',
    }, clear=True)
    def test_signin_oidc_new_user(self):
        email = 'oidc.user@domain.com'
        name = 'OIDC User'
        patch('auth.auth_server.controllers.user.UserController.'
              'domain_blacklist').start()
        with patch('auth.auth_server.controllers.signin.'
                   'OidcOauth2Provider.verify',
                   return_value=(email, name)):
            code, resp = self.client.signin(
                provider='oidc', token='code',
                redirect_uri='https://optscale.example/login')
        self.assertEqual(code, 201)
    ...
    def test_signin_oidc_existing_user(self):
        patch('auth.auth_server.controllers.user.UserController.'
              'domain_blacklist').start()
        token_info = (self.user_customer_email, self.user_customer_name)
        with patch('auth.auth_server.controllers.signin.'
    ...
    def test_signin_oidc_invalid_code(self):
        with patch('auth.auth_server.controllers.signin.'
                   'OidcOauth2Provider.exchange_token',
                   side_effect=ValueError('bad code')):
            code, _ = self.client.signin(
                provider='oidc', token='code',
                redirect_uri='https://optscale.example/login')
        self.assertEqual(code, 403)

    def test_oidc_email_from_preferred_username(self):
        email, name = OidcOauth2Provider._email_and_name({
            'preferred_username': 'oidc.user@domain.com',
            'name': 'OIDC User',
        })
        self.assertEqual(email, 'oidc.user@domain.com')
        self.assertEqual(name, 'OIDC User')

    def test_oidc_loads_from_restapi(self):
        cfg = {
            'configured': True,
            'enabled': True,
            'issuer': 'https://example.onelogin.com/oidc/2',
            'client_id': 'from-ui',
            'client_secret': 'from-ui-secret',
        }
        with patch('auth.auth_server.controllers.signin.RestApiClient') as mock_cls:
            mock_cls.return_value.sso_login_config_get.return_value = (
                200, cfg)
            with patch('auth.auth_server.controllers.signin.Config'):
                provider = OidcOauth2Provider()
                self.assertEqual(provider.client_id(), 'from-ui')
                self.assertEqual(provider.client_secret(), 'from-ui-secret')
                self.assertEqual(
                    provider.issuer(),
                    'https://example.onelogin.com/oidc/2')

    def test_oidc_ui_disabled_blocks_env(self):
        cfg = {
            'configured': True,
            'enabled': False,
            'issuer': '',
